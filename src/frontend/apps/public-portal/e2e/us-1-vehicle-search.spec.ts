...
test.describe('US-1: Vehicle Search with Filters', () => {
  ...
  test.describe('Basic Search Functionality', () => {
    test('should display vehicle search form on homepage', async ({ page }) => {
      ...
      // Check if essential search fields are present
      await expect(page.locator('input[name="pickupDate"], input[formControlName="pickupDate"]')).toBeVisible();
      await expect(page.locator('input[name="returnDate"], input[formControlName="returnDate"]')).toBeVisible();
      await expect(page.locator('select[name="location"], select[formControlName="locationCode"]')).toBeVisible();
    });
    ...
    test('should perform basic vehicle search with dates', async ({ page }) => {
      ...
      // Fill in search form
      await page.fill('input[name="pickupDate"], input[formControlName="pickupDate"]', pickupDate);
      await page.fill('input[name="returnDate"], input[formControlName="returnDate"]', returnDate);

      // Submit search
      await page.click('button:has-text("Suchen"), button[type="submit"]');

      // Wait for results to load
      await page.waitForTimeout(2000);

      // Check if results are displayed
      const resultsVisible = await page.locator('.vehicle-card, .vehicle-result').first().isVisible().catch(() => false);
      ...
      if (resultsVisible) {
        ...
        await expect(page.locator('text=/Keine.*Fahrzeuge|keine.*verfügbar/i')).toBeVisible();
      }
    });
    ...
    test('should display vehicle information in search results', async ({ page }) => {
      ...
      if (cardCount > 0) {
        ...
        const categoryVisible = await firstCard.locator('text=/Kategorie|category/i').isVisible().catch(() => false);
    ...
    test('should validate pickup date is before return date', async ({ page }) => {
      ...
      const hasError = await page.locator('.error, .invalid, text=/ungültig|Fehler/i').isVisible().catch(() => false);
      const buttonDisabled = await page.locator('button:has-text("Suchen"), button[type="submit"]').isDisabled().catch(() => false);
  ...
  test.describe('Location Filter', () => {
    test('should filter vehicles by location', async ({ page }) => {
      ...
      // Select location
      const locationSelect = page.locator('select[name="location"], select[formControlName="locationCode"]');
    ...
    test('should support different pickup and dropoff locations', async ({ page }) => {
      ...
      // Check if separate pickup/dropoff location selects exist
      const hasDropoffLocation = await page.locator('select[name="dropoffLocation"], select[formControlName="dropoffLocationCode"]').isVisible().catch(() => false);

      if (hasDropoffLocation) {
        await page.selectOption('select[name="pickupLocation"], select[formControlName="pickupLocationCode"]', { index: 1 });
        await page.selectOption('select[name="dropoffLocation"], select[formControlName="dropoffLocationCode"]', { index: 2 });
        ...
        const hasResults = await page.locator('.vehicle-card, .vehicle-result').first().isVisible().catch(() => false);
  ...
  test.describe('Category Filter', () => {
    test('should filter vehicles by category', async ({ page }) => {
      ...
      const categoryFilter = page.locator('select[name="category"], select[formControlName="categoryCode"], input[type="checkbox"]:near(text=/Kategorie/i)').first();
    ...
    test('should display all vehicle categories', async ({ page }) => {
      ...
      const categoryFilter = page.locator('select[name="category"], select[formControlName="categoryCode"]');
      const filterVisible = await categoryFilter.isVisible().catch(() => false);
  ...
  test.describe('Fuel Type Filter', () => {
    test('should filter vehicles by fuel type', async ({ page }) => {
      ...
      const fuelFilter = page.locator('select[name="fuelType"], select[formControlName="fuelType"], input[type="checkbox"]:near(text=/Kraftstoff|Benzin|Diesel/i)').first();
      const filterVisible = await fuelFilter.isVisible().catch(() => false);

      if (filterVisible) {
        // Apply fuel filter
        if (await fuelFilter.evaluate(el => el.tagName) === 'SELECT') {
    ...
    test('should display fuel type options (Petrol, Diesel, Electric, Hybrid)', async ({ page }) => {
      const fuelFilter = page.locator('select[name="fuelType"], select[formControlName="fuelType"]');
      const filterVisible = await fuelFilter.isVisible().catch(() => false);
  ...
  test.describe('Transmission Type Filter', () => {
    test('should filter vehicles by transmission type', async ({ page }) => {
      ...
      // Look for transmission filter
      const transmissionFilter = page.locator('select[name="transmission"], select[formControlName="transmissionType"], input[type="checkbox"]:near(text=/Getriebe|Automatik|Manuell/i)').first();
      const filterVisible = await transmissionFilter.isVisible().catch(() => false);

      if (filterVisible) {
        if (await transmissionFilter.evaluate(el => el.tagName) === 'SELECT') {
  ...
  test.describe('Seat Count Filter', () => {
    test('should filter vehicles by minimum seat count', async ({ page }) => {
      ...
      const seatsFilter = page.locator('input[name="minSeats"], input[formControlName="minSeats"], select[name="seats"]');
      const filterVisible = await seatsFilter.isVisible().catch(() => false);

      if (filterVisible) {
        const tagName = await seatsFilter.evaluate(el => el.tagName);

        if (tagName === 'INPUT') {
          await seatsFilter.fill('5');
  ...
  test.describe('Price Display', () => {
    test('should display prices in EUR with VAT included', async ({ page }) => {
      ...
      if (cardCount > 0) {
        ...
        const hasVATMention = await firstCard.locator('text=/MwSt|inkl|VAT/i').isVisible().catch(() => false);
    ...
    test('should display price per day rate', async ({ page }) => {
      ...
      if (cardCount > 0) {
        ...
        const hasDailyRate = await firstCard.locator('text=/Tag|day|täglich/i').isVisible().catch(() => false);
  ...
  test.describe('Filter Combinations', () => {
    test('should apply multiple filters simultaneously', async ({ page }) => {
      ...
      const categoryFilter = page.locator('select[name="category"], select[formControlName="categoryCode"]').first();
      const fuelFilter = page.locator('select[name="fuelType"], select[formControlName="fuelType"]').first();
    ...
    test('should reset all filters', async ({ page }) => {
      ...
      const resetButton = page.locator('button:has-text("Zurücksetzen"), button:has-text("Reset"), button:has-text("Filter löschen")');
  ...
  test.describe('User Interactions', () => {
    test('should navigate to booking page when selecting a vehicle', async ({ page }) => {
      ...
      if (cardCount > 0) {
        ...
        const bookButton = vehicleCards.first().locator('button:has-text("Buchen"), button:has-text("Jetzt buchen"), a:has-text("Details")');
    ...
    test('should display loading state during search', async ({ page }) => {
      ...
      const loadingVisible = await page.locator('.spinner, .loading, [role="progressbar"], text=/Laden|Loading/i').isVisible().catch(() => false);
    ...
    test('should display empty state when no vehicles match filters', async ({ page }) => {
      ...
      if (cardCount === 0) {
        ...
        const emptyStateVisible = await page.locator('text=/Keine.*Fahrzeuge|keine.*verfügbar|keine.*Ergebnisse/i').isVisible().catch(() => false);
