      ...
      <nav class="wizard__nav" role="tablist" [attr.aria-label]="ariaLabel()">
        @for (step of steps(); track step.id; let i = $index) {
          <button
            ...
            [class.wizard__step-indicator--completed]="stepStates()[step.id]?.status === 'completed'"
            [class.wizard__step-indicator--error]="stepStates()[step.id]?.status === 'error'"
            [class.wizard__step-indicator--skipped]="stepStates()[step.id]?.status === 'skipped'"
            [class.wizard__step-indicator--disabled]="step.disabled || (!linear() && !canNavigateToStep(i))"
          ...
          @if (i < steps().length - 1) {
            <div class="wizard__connector" [class.wizard__connector--completed]="stepStates()[step.id]?.status === 'completed'"></div>
          }
        }
      </nav>
            ...
            @if (i === currentIndex() || stepStates()[step.id]?.visited) {
              @if (getStepComponent(step.id); as stepComponent) {
                @if (stepComponent.contentTemplate()) {
                  <ng-container *ngTemplateOutlet="stepComponent.contentTemplate()!"></ng-container>
                }
              }
            }
          </div>
...
export class WizardComponent implements AfterContentInit {
  ...
  readonly canComplete = computed(() => {
    ...
    return this.steps().every(step => {
      ...
      return state?.status === 'completed' || step.id === this.currentStep()?.id;
    }) && this.canProceed();
  });
  ...
  private updateStepState(stepId: string, updates: Partial<WizardStepState>): void {
    const states = this.stepStates();
    this.stepStates.set({
      ...states,
