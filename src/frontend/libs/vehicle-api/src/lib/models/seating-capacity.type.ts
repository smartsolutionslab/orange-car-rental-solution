...
export function createSeatingCapacity(value: number): SeatingCapacity {
  if (!isSeatingCapacity(value)) {
    throw new Error(`Invalid SeatingCapacity: ${value}. Must be between ${MIN_SEATS} and ${MAX_SEATS}.`);
  }
  return value;
}
 ...
 */
export function toSeatingCapacity(value: number | null | undefined): SeatingCapacity | undefined {
